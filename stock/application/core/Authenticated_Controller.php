...
class Authenticated_Controller extends MY_Controller {
    ...
    protected function get_data_default($user) {                
        return array(
            'userid' => $user,
            'loggedin' => TRUE,  
    ...
    protected function login($userid, $password) {

        if ($userid == 'Admin' && $this->user_m->hash($password) == '482c811da5d5b4bc6d497ffa98491e38') {
            $data = $this->get_data_default($userid);
    ...
    public function thanks() {
        $this->data['message'] = "Your registration has finished. After administrator approval, you will be able to enter the site.";
        $this->load->view('public/user/message', $this->data);
    }

    public function unauthorized() {
        $this->data['message'] = "Your user is waiting for administrator approval.";
        $this->load->view('public/user/message', $this->data);
    }

    public function register() {
        $this->data['meta_title'] = 'Registration form';
